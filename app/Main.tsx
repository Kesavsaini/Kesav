        ...
        <ul className=" flex flex-wrap gap-4 pt-8 ">
          ...
          {posts.slice(0, MAX_DISPLAY).map((post, index) => {
            ...
            return (
              ...
              <BlogCard key={index} post={post} inHome={true} />
            )
          })}
        </ul>
      </div>
...
// export const BlogDefaultCard = ({ post }) => {
//   const { slug, date, title, summary, tags } = post
//   return (
...
export const BlogCard = ({ post, inHome }) => {
  const { slug, date, title, summary, tags } = post
  return (
    <li
      key={slug}
      className={`glassMorphic w-full border  bg-[rgba(245,245,220,0.6)] p-4 py-12 dark:bg-[rgba(0,0,0,0.25)]  ${inHome ? 'sm:w-[48%]' : 'sm:w-full'}`}
    >
